...
def assemble_part_solo(
    ...
    *,
    ...
    # Walk shapes. In replica/flat mode, render ALL shapes including placeholders
    # as full content (not just guides). Placeholder guides are only useful for
    # template creation workflows.
    for node in walk_sp_tree(part.xml):
        # Always render full content — placeholders have real text in slide context
        chunk = _convert_node(node, ctx, top_level=True)
        if chunk:
            body_parts.append(chunk)
