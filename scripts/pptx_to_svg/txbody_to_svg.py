...
def convert_txbody(
    ...
    *,
    ...
    # If bodyPr did NOT explicitly specify insets (using defaults) and the text
    # would overflow with those defaults, use zero insets. This matches PPT's
    # behavior for tight-fitting placeholders (titles, labels) where inherited
    # layout/master styles typically override the spec defaults to 0.
    explicit_insets = (body_pr is not None and
                      any(body_pr.get(k) is not None for k in ("lIns", "tIns", "rIns", "bIns")))
    if not explicit_insets:
        # Check if text would overflow with default insets
        inner_h_check = max(xfrm.h - tins - bins, 1.0)
        first_para_font = max((r.font_size_px for p in paragraphs for r in p.runs if r.text), default=DEFAULT_FONT_SIZE_PX)
        if first_para_font * 1.2 > inner_h_check:
            # Text overflows → zero insets (PPT behavior for tight boxes)
            tins = 0.0
            bins = 0.0
            lins = min(lins, 8.0)  # keep small horizontal inset
            rins = min(rins, 8.0)
    ...
    if anchor == "ctr":
        ...
        # Bottom anchor: push text toward bottom of box.
        # BUT: if text overflows (total_h >= inner_h), render from top
        # to avoid pushing text below the visible area. PPT does the same —
        # it never hides text, just lets it overflow upward.
        gap = inner_h - total_h
        if gap > 0:
            cursor_y = inner_y + gap
        else:
            cursor_y = inner_y  # overflow: start from top
    else:
        cursor_y = inner_y

    # Allow generous overflow — PPT shows text even if it exceeds the box
    bottom_y = inner_y + inner_h + total_h  # effectively no clipping
...
def _parse_paragraph(
    ...
    # ROOT FIX: Also check pPr/defRPr — this is the paragraph-level default
    # run style. PowerPoint's inheritance chain is:
    #   run.rPr → endParaRPr → pPr.defRPr → lstStyle.lvlNpPr.defRPr → master txStyles
    # We add defRPr as a third fallback in _build_run's attribute chain.
    def_rpr = None
    if p_pr is not None:
        def_rpr = p_pr.find("a:defRPr", NS)
    # Also check lstStyle in the parent txBody
    if def_rpr is None:
        tx_body = p_elem.getparent() if hasattr(p_elem, 'getparent') else None
        if tx_body is not None:
            lst_style = tx_body.find("a:lstStyle", NS) if hasattr(tx_body, 'find') else None
            if lst_style is not None:
                lvl_tag = f"a:lvl{para.level + 1}pPr"
                lvl_pr = lst_style.find(lvl_tag, NS)
    ...
    for child in list(p_elem):
        ...
        if local == "r":
            ...
            run = _build_run(text, rpr, end_rpr, palette, theme_fonts, def_rpr)
...
def _build_run(
    ...
    When rPr/endParaRPr omit font or size, we fall back to theme_fonts
    (the slide master's default font stack), which is what PowerPoint does.

    Attribute inheritance chain: rPr → endParaRPr → defRPr → theme defaults.
    """
    # font-size: rPr@sz; check full chain including defRPr
    sz = _attr_chain((rpr, end_rpr, def_rpr), "sz")
    font_size_px = hundredths_pt_to_px(sz, DEFAULT_FONT_SIZE_PX)

    # Bold / italic — check full chain
    bold = _attr_chain((rpr, end_rpr, def_rpr), "b") == "1"
    italic = _attr_chain((rpr, end_rpr, def_rpr), "i") == "1"

    # Underline / strike
    u_val = _attr_chain((rpr, end_rpr, def_rpr), "u")
    underline = u_val not in (None, "", "none")
    strike_val = _attr_chain((rpr, end_rpr, def_rpr), "strike")
    strikethrough = strike_val in ("sngStrike", "dblStrike")

    # Letter spacing (rPr@spc, in 1/100 pt)
    spc = _attr_chain((rpr, end_rpr, def_rpr), "spc")
    ...
    # Color — check rPr, endParaRPr, and defRPr for solidFill
    fill = DEFAULT_FILL_HEX
    fill_opacity = 1.0
    color_source = None
    for src in (rpr, end_rpr, def_rpr):
        if src is None:
            continue
    ...
    # Font typeface — check rPr, endParaRPr, defRPr
    latin_face = _typeface(rpr, "latin") or _typeface(end_rpr, "latin") or _typeface(def_rpr, "latin")
    ea_face = _typeface(rpr, "ea") or _typeface(end_rpr, "ea") or _typeface(def_rpr, "ea")
    cs_face = _typeface(rpr, "cs") or _typeface(end_rpr, "cs") or _typeface(def_rpr, "cs")
    ...
    # ROOT FIX: When no font is specified at all (rPr absent or empty),
    # inherit from theme_fonts. PowerPoint uses minorLatin/minorEastAsia
    # as the default body font, and majorLatin/majorEastAsia for titles.
    # Since we can't distinguish title vs body here, use minor (body) default.
    if not latin_face:
        latin_face = theme_fonts.get("minorLatin") or "Arial"
    if not ea_face:
        ea_face = theme_fonts.get("minorEastAsia") or theme_fonts.get("majorEastAsia") or "KaiTi"
...
def _build_font_stack(latin: str | None, ea: str | None, cs: str | None) -> str:
    """Build a unified font-family stack.

    Rules (McKinsey house style):
      - Chinese text: 楷体 (KaiTi) always
      - English titles: Georgia  (latin == Georgia or majorLatin theme ref)
      - English body:  Arial     (everything else)
    The browser picks the first font that has the glyph, so we put the
    Latin font first (for ASCII), then KaiTi (for CJK), then fallbacks.
    """
    # Determine which Latin font to use
    latin_font = "Arial"  # default body
    if latin and latin.lower() in ("georgia", "times new roman", "cambria"):
        latin_font = latin  # title / serif font

    # Fixed stack: Latin font → KaiTi → STKaiti (macOS) → fallback
    return f'{latin_font}, KaiTi, STKaiti, &quot;楷体&quot;, sans-serif'
...
def _char_width(ch: str, font_size: float, bold: bool) -> float:
    ...
    """
    if _is_cjk(ch):
        w = font_size * 0.95  # KaiTi/STKaiti is slightly narrower than full em
    elif ch == ' ':
        w = font_size * 0.3
    elif ch in 'mMwWOQ':
        w = font_size * 0.75
...
def _estimate_run_width(text: str, run: TextRun) -> float:
    return sum(_char_width(c, run.font_size_px, run.bold) for c in text)
...
def _clip_lines_to_bottom(
    ...
    """Return the leading display lines whose baselines fit in the text frame.

    PowerPoint renders text that overflows the text box boundary, so we use a
    generous tolerance: keep a line if its *baseline* (≈ top + 0.85 * font_size)
    is within the box, even if the full line height exceeds the bottom.
    ...
    for i, line in enumerate(lines):
        ...
        # Estimate baseline position (typically ~85% of line height)
        baseline_y = cursor_y + line_h * 0.85
        if i > 0 and baseline_y > bottom_y:
            break
        visible.append(line)
        cursor_y += line_h
    return visible
